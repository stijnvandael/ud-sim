...
//	}
	...
	public void solve(long[] Pmax, long[] Emin, long[] Emax, int quartersLeftInFirstBid, int[] bids, int bidSize, ArrayList<FlexGraph> flexGraphs) {
		solve(convertToDoubles(Pmax), convertToDoubles(Emin), convertToDoubles(Emax), quartersLeftInFirstBid, bids, bidSize, flexGraphs);
	}
	
	public void solve(double[] Pmax, double[] Emin, double[] Emax, int quartersLeftInFirstBid, int[] bids, int bidSize, ArrayList<FlexGraph> flexGraphs) {
		...
		try {
			...
			// path
			IloNumVar[] e = cplex.numVarArray(Emin.length, Emin, Emin);
			...
			IloNumVar[] regPowerQ = cplex.numVarArray(Pmin.length, Pmin, Pmax);
			
			//calculate regulation power in each optimization interval
			for(int i = 0; i < regPowerQ.length; i++) {
				cplex.addEq(regPowerQ[i], cplex.diff(Pmax[i], p[i]));
			}
			...
			for(int i = 0; i < regPowerQ.length; i++) {
				...
				if(i > 0) {
					...
					cplex.addLe(cplex.prod(bidsH[h],bidSize), regPowerQ[i]);	
				}
...
//			System.out.println(bidsH[0].getLB());;
...
//			.setUB(cplex.getValue(bidsH[0]));
			...
			cplex.addLe(cplex.sum(bidsH), totalBids);
